...
def client():
    ...
    return Client(token, plugins=None)
...
def page(client):
    ...
    return client.get_page_or_database(object_id)


def test_from_pandoc_1(client, page):
...
def test_from_pandoc_2(client, page):
    pandoc_ast = page.block.children[-1].to_pandoc()
    client.save_block(page, pandoc_ast)
    new_block_ast = client.unsaved_blocks[page][-1].to_pandoc()
    assert pandoc_ast == new_block_ast
...
def test_from_pandoc_3(client, page):
    pandoc_ast = page.block.children[-1].to_pandoc()
    client.save_block(page, pandoc_ast)
    new_block_ast = client.unsaved_blocks[page][-1].to_pandoc()
    assert pandoc_ast == new_block_ast
